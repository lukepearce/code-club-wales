  ...
  plugins: [
    magicLink({
      ...
      sendMagicLink: async ({ email, url }) => {
        ...
        // The Resend SDK returns API errors in `error` rather than throwing — if
        // we don't check it, a rejected send (e.g. unverified domain) still looks
        // like success. Surface it so sign-in fails loudly instead of silently.
        ...
        if (error) {
          console.error('[auth] Resend failed to send magic link to', email, '-', error);
