...
A mini-site within the wider [Code Club](https://codeclub.org/en) charity. Crew members work through lessons and ship their own personal site at `<slug>.codeclub.wales`. Signup is open; the organiser reviews sites before they are published.

## Language

**Crew member**:
A person who participates in the club. The landing page already names existing ones as "Crew". Anyone can sign up to become one.
...
**Organiser**:
The person who runs the club, reviews submissions, and grants verification. Currently a single role (the user). Plays the same role as the "teacher" in Raspberry Pi Foundation's *Code Classroom*: stands in for the parent under UK-GDPR education exceptions when consent paperwork is collected in person.
...
- **Organiser-provisioned accounts** — v1 is self-signup only. Organiser-provisioned accounts (the "Code Classroom" analogue) deferred to v2.
...
- **Tables**:
  ...
  - `crew_member` — one row per `user`, FK to `user.id`. Holds all domain fields: `display_name`, `dob`, `parent_permission_at` (nullable timestamp — when the under-13 checkbox was clicked, for audit), `slug` (nullable), `slug_claimed_at` (nullable), `slug_verified_at` (nullable).
...
- **Auth model**: Self-signup with email + Google. Under-13 self-attestation with parent-permission notice, following Raspberry Pi Foundation patterns. **Better Auth** for the auth library.
- **Signup flow (v1)**: Single form. Email/password OR Google. Asks date of birth (stored for compliance, not displayed). If DOB under 13, the same form shows a parent-permission notice with a child-friendly link to the privacy policy, plus a checkbox "I have my parent or guardian's permission." Account active immediately on submit — no parental email collection, no consent-link gate. Matches what Raspberry Pi Foundation do for the same audience.
...
- "Name" (the free-text input in `lessons/phase-0-question-cards.html`) is not the canonical identity of a Crew member. Once auth exists, the typed name becomes a display label at most, or is replaced entirely by the Crew member's authenticated identity.
